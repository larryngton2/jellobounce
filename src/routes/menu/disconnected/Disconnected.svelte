<script lang="ts">
    ...
    async function reconnectWithRandomAccount() {
        const account =
            premiumAccounts[Math.floor(Math.random() * premiumAccounts.length)];
        await loginToAccount(account.id);
    }

    onMount(async () => {
        premiumAccounts = (await getAccounts()).filter(
            (a) => a.type !== "Cracked" && !a.favorite,
        );

        setTimeout(() => {
            // TODO: Hacky fix for issues caused by stuck route fix
            listen(
                "accountManagerLogin",
                async (e: AccountManagerLoginEvent) => {
...
<div class="reconnect">
    ...
    <ButtonSetting
        ...
        on:click={reconnectWithRandomAccount}
        disabled={premiumAccounts.length === 0}
    />
    <ButtonSetting
        title="Reconnect with random username"
        on:click={reconnectWithRandomUsername}
    />
</div>
